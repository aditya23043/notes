...
int main() {
    ...
    float complex FFT_input[N] = {11 + 23 * I, 32 + 10 * I, 91 + 94 * I,
        ...
        96 + 17 * I, 49 + 58 * I};
    float complex FFT_input_PS[N];
    float complex FFT_output_sw[N], FFT_output_hw[N];
    float complex FFT_rev_sw[N];

    for (int i = 0; i < N; i = i + 1) {
        FFT_input_PS[i] = 2.0 * crealf(FFT_input[i]) + I * 2.0 * cimagf(FFT_input[i]);
    }
    ...
    // FFT
    bitreverse(FFT_input_PS, FFT_rev_sw);
    ...
    time = 0;
    time = (float)1.0 * (PL_end_time - PL_start_time) /
        (COUNTS_PER_SECOND / 1000000);
