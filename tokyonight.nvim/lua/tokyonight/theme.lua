...
function M.setup()
  ...
  theme.highlights = {
    ...
    LineNr = { fg = c.disable }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    CursorLineNr = { fg = c.orange, bold = true }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    LineNrAbove = { fg = c.disable },
    LineNrBelow = { fg = c.disable },
    MatchParen = { fg = c.parens, bold = true, underline = true }, -- The character under the cursor or just before it, if it is a paired bracket, and its match. |pi_paren.txt|
    ...
    IncSearch = { bg = "#aaaaaa", fg = c.black }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
